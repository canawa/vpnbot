...
async def shout_message(message: Message):
    ...
    try:
        await message.answer(summary, parse_mode='HTML', reply_markup=ikb_back)
    except Exception:
        pass
...
YEAR_OLD_PRICE_BROADCAST_TEXT = (
    '<tg-emoji emoji-id="5420323339723881652">⚠️</tg-emoji> <b>ОСТАЛСЯ 1 ДЕНЬ </b>\n\n'
    'Уже завтра цена на годовой тариф станет выше.\n\n'
    'Сегодня - последний день, когда можно оформить год по текущей цене и не переплачивать потом.\n'
    'Если ты всё это время откладывал — момент уже настал.\n\n'
    '<b> Через сутки это предложение исчезнет. </b>\n\n'
    '<tg-emoji emoji-id="5420315771991497307">🔥</tg-emoji> '
    'Забирай годовой тариф сейчас и фиксируй выгодную цену на 12 месяцев.\n\n'
    '<b> Завтра будет поздно. </b>'
)
...
async def main():
    ...
    async def _spawn_workers_after_polling():
        ...
        logging.info('background workers scheduled')

    asyncio.create_task(_spawn_workers_after_polling())
