...
async def _send_open_invoice_reminder(bot, user_id: int) -> None:
    from aiogram.types import FSInputFile

    if not os.path.isfile(ZHIRIK_VIDEO_PATH):
        logger.error('open invoice reminder: video not found: %s', ZHIRIK_VIDEO_PATH)
        await bot.send_message(user_id, OPEN_INVOICE_REMINDER_TEXT, parse_mode='HTML')
        return
...
async def _open_invoice_reminder_worker(
    ...
    if is_yookassa_payment_settled(payment_id, tx_type):
        return
    try:
        await _send_open_invoice_reminder(bot, user_id)
        logger.info(
...
def schedule_open_invoice_payment_reminder(
    ...
    """Через 3 минуты — видео + текст, если счёт ещё не оплачен."""
    asyncio.create_task(
        _open_invoice_reminder_worker(bot, user_id, str(payment_id).strip(), tx_type),
    )